...
void forward_network(network *netp)
{
    ...
    network net = *netp;
    int i;
    __teamplay_time(loop_cost, 303, high);
    for(i = 0; i < net.n; ++i){
        net.index = i;
        layer l = net.layers[i];
        if(l.delta){
	    __teamplay_worst_time(fill_cpu_time, 1, high);
            fill_cpu(l.outputs * l.batch, 0, l.delta, 1);
        }
        __teamplay_worst_time(forward_time, 8, high);
        l.forward(l, net);
    ...
    __teamplay_assert(loop_cost <= (fill_cpu_time + forward_time + OVERHEAD) * 107);

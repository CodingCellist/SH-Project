...
data BEvald : BooleanExpression -> Nat -> Type where
    MkBEvald : (x : BooleanExpression) -> (y : Bool) -> BEvald x y
        ...
        The less-than-or-equals operator, \texttt{LTE}, is the only operator defined from the ground up, as the other similar operators (i.e. \texttt{LT}, \texttt{GTE}, and \texttt{GT}) can be defined from it. Strictly less-than, \texttt{LT}, can be defined from less-than-or-equals as follows: if the successor of a number $m$ is LTE to another number $n$, then $m$ is strictly less-than $n$; $(m + 1) \leq n \Rightarrow m < n$.
...
\section{Boolean Disjunction (\texttt{Or})}
    ...
    \begin{code}[label={des:or-uninh}, caption={Impossible \texttt{And} cases}]
